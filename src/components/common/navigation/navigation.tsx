...
export const Navigation = ({}) => {
        ...
        <Fragment key={i}>
          <li>
            <Link
              ...
              className={clsx(
                "link-primary",
                item.slug === activeItem ? "link-primary--active" : ""
              )}
            >
              {item.title}
            </Link>
  ...
  const navOnMobile = () => (
    <>
      {isOpen ? (
        <Icon icon="close" size={24} onButtonClick={toggleNavMenu} />
      ) : (
        <Icon icon="burger" size={24} onButtonClick={toggleNavMenu} />
      )}
      <ul
        className={clsx(
          styles["navigation-panel"],
          isOpen ? styles["navigation--open"] : styles["navigation--closed"]
        )}
      >
        <Link
          href="/"
          onClick={toggleNavMenu}
          className={clsx(styles["navigation__domain"], "link-primary")}
        >
          {t("general.header.domain")}
        </Link>
        {items.map((item) => (
